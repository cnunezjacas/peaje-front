...
const emit = defineEmits(['ActualizarTablaComprobante'])
  ...
  if (ComprobarEstadoInputs() != STRINGS.desabilitar) {
    ...
    if (existeCodigo) {
      ...
      try {
        ...
        emit('ActualizarTablaComprobante', true)
      } catch (error) {
        console.error('Error al crear item:', error)
        notify_error(STRINGS.comprobanteAddError)

        emit('ActualizarTablaComprobante', false)
      }
...
/*const InputRegularExpressions = () => {
  if (
    ...
    expRegulares.FullTextAndNumber.test(TextNombre_Comprobante.value.trim())
  )
    return true
  else return false
}*/
