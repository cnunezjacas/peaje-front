  ...
  <div class="q-px-md">
    <div class="flex justify-between padding_minimo">
      <div class="text-center bg-green-10 rounded-borders my-cell">
        <p class="text-tittle-table">{{ STRINGS.gestion }} {{ STRINGS.vehiculosLowercase }}</p>
      </div>
      ...
      <div>
        <q-breadcrumbs>
          ...
          <q-breadcrumbs-el :label="STRINGS.vehiculosLowercase" icon="post_add" />
        </q-breadcrumbs>
      </div>
    </div>
...
const InitDataTable = async () => {
  ...
  try {
    const response = await api.get(STRINGS.urlApiVehiculo)
